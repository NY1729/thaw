...
- ステータス: 単一タグ付き文字列としての例外チャンネルはV1として実装済み
  ...
  **V2(3.1〜3.2節)も実装済み**: `__thaw_pending_exception_object`という並行
  ...
  誤用するとnullポインタ参照でプロセスがクラッシュする(既知の鋭利な角、
  5節参照)。
...
- **`instanceof`ナローイングとの統合を見送った**: 当初は「`if (e instanceof
  MyError)`の分岐内で自動的に`e.code`が読める」ことを目指していたが、
  `union_narrowings`(既存の型ナローイング機構、
  [statements/narrowing.rs](../../crates/thaw-hir/src/lower/statements/narrowing.rs)、
  約700行)は「変数が実際にユニオン型である」ことを前提にしており、
  `e`は常に`HirType::Str`固定(3.2節の前提)なのでこの機構にそのまま乗せる
  ことができなかった。代わりに、TypeScript自身の`as`と同じ「明示的・
...
V1(3.3節・フィールドなし〜ありの両方のクラス継承)・V2(3.1〜3.2節の
`_object`並行チャンネル、ただし`as`ベース)ともに実装済み。残っているのは
5節に挙げた「自動ナローイング統合」と「無検査`as`の安全性強化」で、
どちらも今すぐ必須ではない改善項目として次のステップに位置づける。
