...
双方から観測できる。 名前付きローカル関数式が自己参照する場合はinitializer評価前に専用self cellを確保して生成closureを格納するため、外側binding名と異なる内部名でも再帰し、typed境界やbind後も同じself closureを参照する。 generic関数式はtemplateと内部自己名を同時に保持し、推論・明示型タプルまたはcallback contextごとのmonomorphic closure生成時に具体型self cellへ結び直す。nested closureが作成元の呼び出しを抜けた後もcellは有効である。
...
subpathも候補mapへ加える。実行時のquery/fragmentはbase候補を解決した上でsuffix込みの
cache keyを作り、同じURLは再利用しつつ異なるURLを別instanceとして初期化する。user moduleの
