...
対応するとしたら「実際に使われている範囲だけの `util` 等の最小限の
polyfill を用意する」ことになるが、これは「パッケージの依存グラフを
辿ってバンドルする」という10章までの性質とは異なり、「Node.js
標準ライブラリの一部を自分で実装し直す」という、別種の（潜在的に
大きな）作業になる。優先度は高いが、着手は次の設計判断として
別途行う。
