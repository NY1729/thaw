...
## 19. その他、今回やらなかったこと（意図的なスコープ外）

- **バージョングラフ全体の解決**: 17章の通り、パッケージ1つぶんの
  バージョン指定はできるようになったが、`package-lock.json` 相当の
  依存関係全体のバージョン整合はまだ見ていない。
...
- **その他のプラットフォームグローバル**: `Buffer`/`URL`/`process`
  ...
  網羅的な対応表は用意していない。
- Marshal アダプタの残り: 19章の通り、パラメータ側の `(ptr, len)`/
...
依存グラフ全体のバージョン整合やネイティブアドオンの実行そのものなど、
まだ埋まっていない穴はある（19章）。優先順位は「実際に試して見つかった
順」で決めていく。
