...
fn pure_numeric_registry_export_uses_jit_without_quickjs() {
    ...
    std::fs::write(
        ...
        "'use strict'; const SCALE = 2; const double = value => value * SCALE; module.exports = { add: function(left, right) { const sum = left + right; const doubled = sum * SCALE; const delta = left - right; if (left < right) return doubled; return delta; }, accumulate: function(left, right) { let total = left; total += right; total *= SCALE; total--; return total; }, choose: function(left, right) { if (left) { if (right < 0) return 1; return 2; } else if (right) return 3; else return 4; }, sub: (left, right) => left - right, double, negate: value => -value, mask: value => (value & 255) ^ 42, fallbackOr: (value, fallback) => value || fallback, guard: (value, result) => value && result, magnitude: value => Math.abs(value), roundedRoot: value => Math.round(Math.sqrt(value)), logarithm: value => value + Math.round(Math.PI), integerMath: (left, right) => Math.imul(left, right) + Math.clz32(1) + Math.fround(1), stringLength: value => value.length, stringLess: (left, right) => left < right, greet: value => 'hello, ' + value + '!', matches: value => value.startsWith('pre') && value.endsWith('fix') && value.includes('ref'), find: value => value.indexOf('😀') + value.lastIndexOf('😀'), transform: value => value.toLowerCase().toUpperCase(), power: (base, exponent) => Math.pow(base, exponent), less: (left, right) => left < right, negateFlag: value => !value, remainder: (left, right) => left % right, minimum: (a, b, c) => Math.min(a, b, c), maximum: (a, b, c) => Math.max(a, b, c), sum3: (a, b, c) => a + b + c, answer: () => Math.round(Math.PI) + 39 };\n",
    )
    .unwrap();
    let entry = dir.join("main.ts");
    std::fs::write(
        &entry,
        "import { add, accumulate, choose, sub, double, negate, mask, fallbackOr, guard, magnitude, roundedRoot, logarithm, integerMath, stringLength, stringLess, greet, matches, find, transform, power, less, negateFlag, remainder, minimum, maximum, sum3, answer } from 'jit-math';\nfunction main(): void { console.log(add(2, 19) + accumulate(1, 2) + choose(1, -1) + sub(1, 1) + double(0) + negate(0) + mask(42) + fallbackOr(0, 42) + guard(0, 42) + magnitude(-1) + roundedRoot(0) + logarithm(8) + integerMath(3, 4) + stringLength('😀') + (stringLess('a', 'b') ? 1 : 0) + stringLength(greet('x')) + (matches('prefix') ? 1 : 0) + find('😀a😀') + stringLength(transform('Ab')) + power(2, 0) + (less(1, 2) && negateFlag(false) ? 1 : 0) + remainder(5, 2) + minimum(-1, 0, 1) + maximum(-1, 0, 1) + sum3(0, 0, 0) + answer() - 167); }\n",
    )
    .unwrap();
    let output = dir.join("app");
    build(&entry, &output, &[], &[], &[], &registry, &[]).unwrap();
