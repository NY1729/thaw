...
fn nested_object_destructuring_uses_jit_without_quickjs() {
    ...
    let source = concat!(
        ...
        "function makeHelperForReturn(values) { let index = 0; for (; index < values.length; index++) { if (values[index] > 2) return { index, label: 'found' }; } return { index: -1, label: 'missing' }; } ",
        ...
        .enumerate()
    {
        ...
        } else if matches!(name, "callHelperWhileReturn" | "callHelperForReturn") {
            let expression = expression.unwrap();
