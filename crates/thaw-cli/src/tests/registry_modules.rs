...
fn numeric_predicates_use_jit_without_quickjs() {
    ...
    std::fs::write(
        package.join("bundle.js"),
        "module.exports = { safe: value => Number.isSafeInteger(value), stringNan: value => Number.isNaN(value) || isNaN(value), float: value => Number.parseFloat(value), integer: (value, radix) => parseInt(value, radix), fixed: (value, digits) => value.toFixed(digits), precision: (value, digits) => value.toPrecision(digits), radix: (value, base) => value.toString(base), exponential: (value, digits) => value.toExponential(digits), exponentialShortest: value => value.toExponential(), count: values => values.length, pick: (values, index) => values.at(index), pickText: (values, index) => values.at(index), pickBool: (values, index) => values.at(index), hasNumber: (values, needle, from) => values.includes(needle, from), hasText: (values, needle, from) => values.includes(needle, from), boolIndex: (values, needle, from) => values.indexOf(needle, from), lastNumber: (values, needle) => values.lastIndexOf(needle), lastText: (values, needle, from) => values.lastIndexOf(needle, from), lastBool: (values, needle, from) => values.lastIndexOf(needle, from) };\n",
    )
    .unwrap();
    let entry = dir.join("main.ts");
    std::fs::write(
        &entry,
        "import { safe, stringNan, float, integer, fixed, precision, radix, exponential, exponentialShortest, count, pick, pickText, pickBool, hasNumber, hasText, boolIndex, lastNumber, lastText, lastBool } from 'jit-predicates';\nfunction main(): void { console.log(safe(42)); console.log(stringNan('x')); console.log(float('  -12.5px')); console.log(integer('11', 2)); console.log(fixed(12.5, 2)); console.log(precision(12.5, 3)); console.log(radix(255, 16)); console.log(exponential(12.6, 1)); console.log(exponentialShortest(12.5)); console.log(count([1, 2, 3])); console.log(pick([1, 2, 3], -1) ?? 0); console.log(pickText(['a', 'b'], -1) ?? 'none'); console.log(pickBool([false, true], -1) ?? false); console.log(hasNumber([NaN], NaN, 0)); console.log(hasText(['a', 'b'], 'b', 0)); console.log(boolIndex([false, true], true, 0)); console.log(lastNumber([1, 2, 1], 1)); console.log(lastText(['a', 'b', 'a'], 'a', 1)); console.log(lastBool([true, false, true], true, 1)); try { fixed(1, 101); } catch { console.log('range'); } try { exponential(1, 101); } catch { console.log('exp-range'); } }\n",
    )
    .unwrap();
    let output = dir.join("app");
    build(&entry, &output, &[], &[], &[], &registry, &[]).unwrap();
