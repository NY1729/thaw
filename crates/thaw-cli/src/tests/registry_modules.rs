...
fn pure_numeric_registry_export_uses_jit_without_quickjs() {
    ...
    std::fs::write(
        ...
        "module.exports = { add: function(left, right) { const sum = left + right; const doubled = sum * 2; const delta = left - right; if (left < right) return doubled; return delta; }, sub: (left, right) => left - right, double: value => value * 2, negate: value => -value, magnitude: value => Math.abs(value), remainder: (left, right) => left % right, sum3: (a, b, c) => a + b + c, answer: () => 42 };\n",
    )
    .unwrap();
    let entry = dir.join("main.ts");
    std::fs::write(
        &entry,
        "import { add, sub, double, negate, magnitude, remainder, sum3, answer } from 'jit-math';\nfunction main(): void { console.log(add(2, 19) + sub(1, 1) + double(0) + negate(0) + magnitude(-1) + remainder(5, 2) + sum3(0, 0, 0) + answer() - 44); }\n",
    )
    .unwrap();
    let output = dir.join("app");
    build(&entry, &output, &[], &[], &[], &registry, &[]).unwrap();
