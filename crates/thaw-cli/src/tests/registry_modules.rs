...
fn primitive_array_comparisons_use_jit_without_quickjs() {
    ...
    std::fs::write(
        package.join("bundle.js"),
        "module.exports = { selectStrings: (values, minimum) => values.filter(value => value >= minimum), hasFlag: (values, expected) => values.some(value => value === expected) };\n",
    )
    .unwrap();
    let entry = dir.join("main.ts");
    std::fs::write(
        &entry,
        "import { selectStrings, hasFlag } from 'jit-primitive-comparisons';\nfunction main(): void { console.log(selectStrings(['a', 'c', 'b'], 'b').join(',')); console.log(hasFlag([false, true], true)); }\n",
    )
    .unwrap();
    let output = dir.join("app");
    build(&entry, &output, &[], &[], &[], &registry, &[]).unwrap();
    ...
    assert_eq!(String::from_utf8_lossy(&result.stdout), "c,b\ntrue\n");
    let _ = std::fs::remove_dir_all(dir);
}
