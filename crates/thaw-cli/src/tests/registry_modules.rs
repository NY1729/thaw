...
fn aggregate_union_narrows_array_then_dictionary_in_jit() {
    ...
    std::fs::write(
        package.join("bundle.js"),
        "function describe(value) { if (Array.isArray(value)) return String(value.length); if (typeof value === 'object') return String(value.count); return value.toUpperCase(); } function multiSize(value) { if (Array.isArray(value)) return value.length; return value.length; } function multiJoin(value) { if (Array.isArray(value)) return value.join('|'); return value.toUpperCase(); } function multiText(value) { if (Array.isArray(value)) return value.toString(); return value.toUpperCase(); } function multiSlice(value) { if (Array.isArray(value)) return value.slice(1); return value.slice(1); } function multiConcat(value) { if (Array.isArray(value)) return value.concat(value); return value.concat(value); } module.exports = { describe, multiSize, multiJoin, multiText, multiSlice, multiConcat };\n",
    )
    .unwrap();
    let entry = dir.join("main.ts");
    std::fs::write(
        &entry,
        "import { describe, multiSize, multiJoin, multiText, multiSlice, multiConcat } from 'jit-staged-aggregate-narrowing'; function main(): void { const record: Record<string, number> = { count: 42 }; console.log(describe([1, 2, 3])); console.log(describe(record)); console.log(describe('word')); console.log(multiSize([1, 2])); console.log(multiSize(['a', 'b', 'c'])); console.log(multiSize([true])); console.log(multiSize('word')); console.log(multiJoin([1, 2])); console.log(multiJoin(['a', 'b'])); console.log(multiJoin([true, false])); console.log(multiJoin('word')); console.log(multiText([1, 2])); console.log(multiText(['a', 'b'])); console.log(multiText([true, false])); console.log(multiSize(multiSlice([1, 2, 3]))); console.log(multiSize(multiSlice(['a', 'b', 'c']))); console.log(multiSize(multiSlice([true, false]))); console.log(multiSize(multiSlice('word'))); console.log(multiSize(multiConcat([1, 2, 3]))); console.log(multiSize(multiConcat(['a', 'b', 'c']))); console.log(multiSize(multiConcat([true, false]))); console.log(multiSize(multiConcat('word'))); }\n",
    )
    .unwrap();
    let output = dir.join("app");
    build(&entry, &output, &[], &[], &[], &registry, &[]).unwrap();
