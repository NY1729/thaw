...
fn logical_expressions_short_circuit_in_jit_without_quickjs() {
    ...
    std::fs::write(
        package.join("bundle.js"),
        "module.exports.orPop = values => values.pop() || values.pop(); module.exports.andPop = values => values.pop() && values.pop(); module.exports.nested = values => values.pop() || values.pop() || values.pop(); module.exports.embedded = values => (values.pop() || values.pop()) + values.length; module.exports.conditional = (values, chooseLast) => (chooseLast ? values.pop() : values.shift()) + values.length; module.exports.nestedConditional = (values, mutate, chooseLast) => (mutate ? (chooseLast ? values.pop() : values.shift()) : values[1]) + values.length; module.exports.object = values => ({ value: values.pop() || values.pop(), remaining: values.length });\n",
    )
    .unwrap();
    let entry = dir.join("main.ts");
    std::fs::write(
        &entry,
        "import { orPop, andPop, nested, embedded, conditional, nestedConditional, object } from 'jit-logical';\nfunction main(): void { const orHit = [1, 2, 3]; console.log(orPop(orHit) + ':' + orHit.length); const orMiss = [1, 2, 0]; console.log(orPop(orMiss) + ':' + orMiss.length); const andMiss = [1, 2, 0]; console.log(andPop(andMiss) + ':' + andMiss.length); const andHit = [1, 2, 3]; console.log(andPop(andHit) + ':' + andHit.length); const nestedValues = [1, 4, 0, 0]; console.log(nested(nestedValues) + ':' + nestedValues.length); const nestedHit = [1, 2, 3]; console.log(nested(nestedHit) + ':' + nestedHit.length); const embeddedValues = [1, 2, 3]; console.log(embedded(embeddedValues) + ':' + embeddedValues.length); const trueValues = [1, 2, 3]; console.log(conditional(trueValues, true) + ':' + trueValues.join('|')); const falseValues = [1, 2, 3]; console.log(conditional(falseValues, false) + ':' + falseValues.join('|')); const nestedTrue = [1, 2, 3]; console.log(nestedConditional(nestedTrue, true, true) + ':' + nestedTrue.join('|')); const nestedFalse = [1, 2, 3]; console.log(nestedConditional(nestedFalse, false, true) + ':' + nestedFalse.join('|')); const objectValues = [1, 5, 0]; const result = object(objectValues); console.log(result.value + ':' + result.remaining + ':' + objectValues.length); }\n",
    )
    .unwrap();
    let output = dir.join("app");
    build(&entry, &output, &[], &[], &[], &registry, &[]).unwrap();
