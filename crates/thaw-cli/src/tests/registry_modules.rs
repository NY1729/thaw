...
#[test]
fn arithmetic_array_reduce_uses_jit_without_quickjs() {
    let dir = std::env::temp_dir().join(format!(
    ...
    std::fs::write(
        package.join("bundle.js"),
        "module.exports = { sum: (values, initial) => values.reduce(function(accumulator, value) { return accumulator + value; }, initial), subtract: (values, initial) => values.reduce((accumulator, value) => accumulator - value, initial), multiply: (values, initial) => values.reduce((accumulator, value) => accumulator * value, initial), divide: (values, initial) => values.reduce((accumulator, value) => accumulator / value, initial), remainder: (values, initial) => values.reduce((accumulator, value) => accumulator % value, initial), power: (values, initial) => values.reduce((accumulator, value) => accumulator ** value, initial) };\n",
    )
    .unwrap();
    let entry = dir.join("main.ts");
    std::fs::write(
        &entry,
        "import { sum, subtract, multiply, divide, remainder, power } from 'jit-array-reduce';\nfunction main(): void { console.log(sum([1e16, -1e16, 1], 1)); console.log(Object.is(sum([], -0), -0)); console.log(sum([10, 20, 12], 0)); console.log(subtract([1, 2, 3], 10)); console.log(multiply([2, 3, 4], 1)); console.log(divide([2, 5], 100)); console.log(remainder([6, 4], 20)); console.log(power([3, 2], 2)); console.log(Object.is(power([3], -0), -0)); }\n",
    )
    .unwrap();
    let output = dir.join("app");
    build(&entry, &output, &[], &[], &[], &registry, &[]).unwrap();
