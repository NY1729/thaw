...
fn primitive_array_comparisons_use_jit_without_quickjs() {
    ...
    std::fs::write(
        package.join("bundle.js"),
        "module.exports = { selectStrings: (values, minimum) => values.filter(value => value >= minimum), hasFlag: (values, expected) => values.some(value => value === expected), copyStrings: values => values.map(value => value), invertFlags: values => values.map(value => !value) };\n",
    )
    .unwrap();
    let entry = dir.join("main.ts");
    std::fs::write(
        &entry,
        "import { selectStrings, hasFlag, copyStrings, invertFlags } from 'jit-primitive-comparisons';\nfunction main(): void { console.log(selectStrings(['a', 'c', 'b'], 'b').join(',')); console.log(hasFlag([false, true], true)); console.log(copyStrings(['x', 'y']).join(',')); console.log(invertFlags([false, true]).join(',')); }\n",
    )
    .unwrap();
    let output = dir.join("app");
    build(&entry, &output, &[], &[], &[], &registry, &[]).unwrap();
