...
fn mixed_array_range_updates_use_jit_without_quickjs() {
    ...
    std::fs::write(
        package.join("bundle.js"),
        "function fill(value, replacement) { if (Array.isArray(value)) return value.fill(replacement, 1); return value; } function copy(value) { if (Array.isArray(value)) return value.copyWithin(0, 1); return value; } function withValue(value, replacement) { if (Array.isArray(value)) return value.with(1, replacement); return value; } function insert(value, replacement) { if (Array.isArray(value)) return value.push(replacement); return 0; } function prepend(value, replacement) { if (Array.isArray(value)) return value.unshift(replacement); return 0; } function popValue(value) { if (Array.isArray(value)) return value.pop(); return value; } function shiftValue(value) { if (Array.isArray(value)) return value.shift(); return value; } function setValue(value, index, replacement) { if (Array.isArray(value)) return value[index] = replacement; return replacement; } module.exports = { fill, copy, withValue, insert, prepend, popValue, shiftValue, setValue };\n",
    )
    .unwrap();
    let entry = dir.join("main.ts");
    std::fs::write(
        &entry,
        "import { fill, copy, withValue, insert, prepend, popValue, shiftValue, setValue } from 'jit-mixed-array-range'; function main(): void { console.log(fill([1, 2, 3], 9)); console.log(fill(['a', 'b', 'c'], 'x')); console.log(fill([true, false, false], true)); console.log(copy([1, 2, 3])); console.log(copy(['a', 'b', 'c'])); console.log(copy([true, false, false])); console.log(withValue([1, 2, 3], 9)); console.log(withValue(['a', 'b', 'c'], 'x')); console.log(withValue([true, false, false], true)); console.log(insert([1, 2], 9)); console.log(insert(['a', 'b'], 'x')); console.log(insert([true, false], true)); console.log(prepend([1, 2], 9)); console.log(prepend(['a', 'b'], 'x')); console.log(prepend([true, false], true)); console.log(popValue([1, 2])); console.log(popValue(['a', 'b'])); console.log(popValue([true, false])); console.log(shiftValue([1, 2])); console.log(shiftValue(['a', 'b'])); console.log(shiftValue([true, false])); console.log(setValue([1, 2], 1, 9)); console.log(setValue(['a', 'b'], 1, 'x')); console.log(setValue([true, false], 1, true)); }\n",
    )
    .unwrap();
    let output = dir.join("app");
    build(&entry, &output, &[], &[], &[], &registry, &[]).unwrap();
