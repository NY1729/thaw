...
fn aggregate_unions_round_trip_through_jit_without_quickjs() {
    ...
    std::fs::write(
        package.join("package.d.ts"),
        "export declare function arrayIdentity(value: number[] | string): number[] | string;\nexport declare function arrayKind(value: number[] | string): string;\nexport declare function recordIdentity(value: Record<string, number> | string): Record<string, number> | string;\nexport declare function recordKind(value: Record<string, number> | string): string;\nexport declare function text(value: number[] | string[] | Record<string, number> | string): string;\nexport declare function chooseArray(mode: number): string;\nexport declare function chooseAggregate(mode: number): string;\nexport declare function chooseLocal(flag: boolean): string;\n",
    )
    .unwrap();
    std::fs::write(
        package.join("bundle.js"),
        "function arrayIdentity(value) { return value; } function arrayKind(value) { return typeof value; } function recordIdentity(value) { return value.valueOf(); } function recordKind(value) { return typeof value; } function text(value) { return String(value); } function chooseArray(mode) { return String(mode === 0 ? [1, 2] : mode === 1 ? ['a', 'b'] : 'x'); } function chooseAggregate(mode) { return String(mode === 0 ? [1] : mode === 1 ? { count: 2 } : 'x'); } function chooseLocal(flag) { let result = flag ? [1, 2] : 'start'; if (!flag) result = { count: 3 }; return String(result); } module.exports = { arrayIdentity, arrayKind, recordIdentity, recordKind, text, chooseArray, chooseAggregate, chooseLocal };\n",
    )
    .unwrap();
    let entry = dir.join("main.ts");
    std::fs::write(
        &entry,
        "import { arrayIdentity, arrayKind, recordIdentity, recordKind, text, chooseArray, chooseAggregate, chooseLocal } from 'jit-aggregate-unions'; function showArray(value: number[] | string): void { if (typeof value === 'string') console.log(value.toUpperCase()); else console.log(value.length); } function showRecord(value: Record<string, number> | string): void { if (typeof value === 'string') console.log(value.toUpperCase()); else console.log(value.count); } function main(): void { const record: Record<string, number> = { count: 42 }; showArray(arrayIdentity([1, 2, 3])); showArray(arrayIdentity('array')); console.log(arrayKind([1])); console.log(arrayKind('x')); showRecord(recordIdentity(record)); showRecord(recordIdentity('record')); console.log(recordKind(record)); console.log(recordKind('x')); console.log(text([1, 2])); console.log(text(record)); console.log(text('kept')); console.log(chooseArray(0)); console.log(chooseArray(1)); console.log(chooseAggregate(0)); console.log(chooseAggregate(1)); console.log(chooseAggregate(2)); console.log(chooseLocal(true)); console.log(chooseLocal(false)); }\n",
    )
    .unwrap();
    let output = dir.join("app");
    build(&entry, &output, &[], &[], &[], &registry, &[]).unwrap();
