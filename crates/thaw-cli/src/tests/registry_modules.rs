...
fn pure_numeric_registry_export_uses_jit_without_quickjs() {
    ...
    std::fs::write(
        ...
        "module.exports = { add: function(left, right) { const sum = left + right; const doubled = sum * 2; const delta = left - right; if (left < right) return doubled; return delta; }, sub: (left, right) => left - right, double: value => value * 2, negate: value => -value, mask: value => (value & 255) ^ 42, fallbackOr: (value, fallback) => value || fallback, guard: (value, result) => value && result, magnitude: value => Math.abs(value), roundedRoot: value => Math.round(Math.sqrt(value)), power: (base, exponent) => Math.pow(base, exponent), less: (left, right) => left < right, remainder: (left, right) => left % right, minimum: (a, b, c) => Math.min(a, b, c), maximum: (a, b, c) => Math.max(a, b, c), sum3: (a, b, c) => a + b + c, answer: () => 42 };\n",
    )
    .unwrap();
    let entry = dir.join("main.ts");
    std::fs::write(
        &entry,
        "import { add, sub, double, negate, mask, fallbackOr, guard, magnitude, roundedRoot, power, less, remainder, minimum, maximum, sum3, answer } from 'jit-math';\nfunction main(): void { console.log(add(2, 19) + sub(1, 1) + double(0) + negate(0) + mask(42) + fallbackOr(0, 42) + guard(0, 42) + magnitude(-1) + roundedRoot(0) + power(2, 0) + (less(1, 2) ? 1 : 0) + remainder(5, 2) + minimum(-1, 0, 1) + maximum(-1, 0, 1) + sum3(0, 0, 0) + answer() - 88); }\n",
    )
    .unwrap();
    let output = dir.join("app");
    build(&entry, &output, &[], &[], &[], &registry, &[]).unwrap();
