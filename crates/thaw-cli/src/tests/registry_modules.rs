...
fn primitive_dictionaries_use_jit_without_quickjs() {
    ...
    std::fs::write(
        package.join("bundle.js"),
        "function readNumber(values, key) { return values[key] + values.fixed; } function readBool(values, key) { return values[key] && values.enabled; } function readString(values, key) { return values.prefix + values[key]; } function updateNumber(values, key, value) { values[key] = value; values.fixed++; return values[key] + values.fixed; } function updateBool(values, key, value) { values[key] = value; return values[key]; } function updateString(values, key, value) { values[key] += value; return values[key]; } function removeBool(values, key) { delete values[key]; return !values[key]; } function updateAndReturn(values, key, value) { values[key] = value; return values; } function makeNumbers(value) { return { answer: value, doubled: value * 2 }; } function makeBools(value) { return { chosen: value, inverse: !value }; } function makeStrings(value) { return { prefix: 'th', suffix: value }; } function makeDynamic(key, value, base) { return { first: 1, ...base, [key]: value, first: 2 }; } function keys(values) { return Object.keys(values); } function keyCount(values) { return Object.keys(values).length; } function has(values, key) { return Object.hasOwn(values, key); } module.exports = { readNumber, readBool, readString, updateNumber, updateBool, updateString, removeBool, updateAndReturn, makeNumbers, makeBools, makeStrings, makeDynamic, keys, keyCount, has };\n",
    )
    .unwrap();
