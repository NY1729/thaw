...
use swc_ecma_ast::{
    Accessibility, Class, ClassMember, Decl, DefaultDecl, Expr, Function, MemberProp, MethodKind,
    Module, ModuleDecl, ModuleItem, ParamOrTsParamProp, Pat, PropName, TruePlusMinus,
    TsCallSignatureDecl, TsEntityName, TsFnOrConstructorType, TsFnParam, TsFnType, TsInterfaceDecl,
    TsKeywordTypeKind, TsLit, TsMethodSignature, TsNamespaceBody, TsParamPropParam, TsType,
...
pub struct DtsGenericFunction {
    ...
    /// A generic tuple return rendered as TypeScript for the fallback shim.
    /// Opaque `JsValue` members are rejected rather than silently converted
    /// to JSON, preserving object identity.
    pub tuple_return_type: Option<String>,
}
