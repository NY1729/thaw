...
pub fn resolve_builtin(specifier: &str) -> Result<ResolvedPackage, String> {
    ...
    let dts_source = match name {
        ...
        "stream" => {
            // A deliberately practical (not exhaustive) ambient surface
            // for the real, already-fully-implemented JS runtime
            // (`registry/builtins/streams/core.rs` -- a faithful
            // `Readable`/`Writable`/`Duplex`/`Transform`/`PassThrough`,
            // including `.read()`/`.write()`/`.pipe()`/async iteration/
            // `.map()`/`.filter()`/etc.): previously declared only a
            // handful of methods per class with NO real `extends`
            // relationship between any of them (each a flat, standalone
            // declaration), so a *third-party package's own* class
            // extending one of these (real example: csv-parse's `Parser
            // extends stream.Transform`) inherited nothing at all beyond
            // that handful -- most critically, no `Readable.prototype.
            // read`, the single most common method a `Transform`-based
            // package's consumer actually calls (`while ((chunk =
            // stream.read()) !== null) { ... }`, Node's own documented
            // idiom). Fixed by adding the missing widely-used methods,
            // matching the runtime's own real surface, and real
            // `extends` clauses mirroring the runtime's actual prototype
            // chain (`Duplex extends Readable`, `Transform extends
            // Duplex`, `PassThrough extends Transform`) so inheritance
            // resolves correctly through `thaw-bridge`'s existing same-
            // file class-extends mechanism. `Writable`'s own members are
            // still repeated directly on `Duplex` (TypeScript has no
            // multiple inheritance for classes) rather than modeled via
            // a second `extends`, matching how the runtime itself copies
            // `Writable.prototype`'s methods onto `Duplex`/`Transform`.
            "export declare function Stream(argsArray: any): any;\nexport declare class Readable { constructor(options?: any); static from(value: Json, options?: any): Readable; on(event: string, listener: (value: JsValue) => void): Readable; on(event: string, listener: () => void): Readable; on(event: string, listener: (...args: any[]) => void): Readable; once(event: string, listener: (value: JsValue) => void): Readable; once(event: string, listener: () => void): Readable; off(event: string, listener: (...args: any[]) => void): Readable; removeListener(event: string, listener: (...args: any[]) => void): Readable; emit(event: string, ...args: any[]): boolean; pipe(destination: JsValue, options?: any): JsValue; unpipe(destination?: JsValue): Readable; pause(): Readable; resume(): Readable; isPaused(): boolean; read(size?: number): Json; setEncoding(encoding: string): Readable; push(chunk: Json, encoding?: string): boolean; unshift(chunk: Json, encoding?: string): boolean; destroy(error?: any): Readable; wrap(source: JsValue): Readable; readable: boolean; readableEnded: boolean; readableFlowing: boolean | null; readableHighWaterMark: number; readableLength: number; destroyed: boolean; closed: boolean; errored: any; }\nexport interface WritableOptions { write?: (chunk: JsValue, encoding: string, callback: () => void) => void; }\nexport interface DuplexOptions { read?: () => void; write?: (chunk: JsValue, encoding: string, callback: () => void) => void; }\nexport interface TransformOptions { transform?: (chunk: JsValue, encoding: string, callback: (error?: any, data?: Json) => void) => void; }\nexport interface TransformFlushOptions { transform?: (chunk: JsValue, encoding: string, callback: (error?: any, data?: Json) => void) => void; flush?: (callback: (error?: any, data?: Json) => void) => void; }\nexport declare class Writable { constructor(options?: WritableOptions); on(event: string, listener: (value: JsValue) => void): Writable; on(event: string, listener: () => void): Writable; on(event: string, listener: (...args: any[]) => void): Writable; once(event: string, listener: (value: JsValue) => void): Writable; once(event: string, listener: () => void): Writable; off(event: string, listener: (...args: any[]) => void): Writable; removeListener(event: string, listener: (...args: any[]) => void): Writable; emit(event: string, ...args: any[]): boolean; write(chunk: Json, encoding?: string, callback?: (error?: any) => void): boolean; end(chunk?: Json, encoding?: string, callback?: () => void): Writable; cork(): void; uncork(): void; destroy(error?: any): Writable; setDefaultEncoding(encoding: string): Writable; writable: boolean; writableEnded: boolean; writableFinished: boolean; writableHighWaterMark: number; writableLength: number; destroyed: boolean; }\nexport declare class Duplex extends Readable { constructor(options?: DuplexOptions); write(chunk: Json, encoding?: string, callback?: (error?: any) => void): boolean; end(chunk?: Json, encoding?: string, callback?: () => void): Duplex; cork(): void; uncork(): void; destroy(error?: any): Duplex; setDefaultEncoding(encoding: string): Duplex; writable: boolean; writableEnded: boolean; writableFinished: boolean; writableHighWaterMark: number; writableLength: number; }\nexport declare class Transform extends Duplex { constructor(); constructor(options: TransformOptions); constructor(options: TransformFlushOptions); }\nexport declare class PassThrough extends Transform { constructor(options?: any); }\nexport declare function pipeline(source: JsValue, destination: JsValue, callback: (error: any) => void): void;\nexport declare function compose(argsArray: any): any;\nexport declare function finished(stream: JsValue, callback: (error: any) => void): JsValue;\nexport declare function finished(stream: JsValue, options: any, callback: (error: any) => void): JsValue;\nexport declare function addAbortSignal(argsArray: any): any;\nexport declare function isDestroyed(argsArray: any): boolean;\nexport declare function isDisturbed(argsArray: any): boolean;\nexport declare function isErrored(argsArray: any): boolean;\nexport declare function isReadable(argsArray: any): boolean;\nexport declare function isWritable(argsArray: any): boolean;\nexport declare function getDefaultHighWaterMark(argsArray: any): number;\nexport declare function setDefaultHighWaterMark(argsArray: any): void;\nexport declare function duplexPair(argsArray: any): any;\nexport declare function destroy(argsArray: any): void;\nexport declare function _isArrayBufferView(argsArray: any): boolean;\nexport declare function _isUint8Array(argsArray: any): boolean;\nexport declare function _uint8ArrayToBuffer(argsArray: any): any;\nexport declare const promises: any;\n"
        }
        "stream/promises" => {
        ...
        "crypto" => {
            "export declare class Buffer { length: number; toString(encoding?: string): string; }\nexport declare class Hash { update(data: string): Hash; digest(encoding: string): string; copy(): Hash; }\nexport declare class Hmac { update(data: string): Hmac; digest(encoding: string): string; }\nexport declare class Cipheriv { update(data: Json, inputEncoding?: string, outputEncoding?: string): Buffer; final(outputEncoding?: string): Buffer; }\nexport declare class KeyObject { type: string; asymmetricKeyType?: string; }\nexport declare class Sign { update(data: Json, encoding?: string): Sign; sign(privateKey: Json, outputEncoding?: string): JsValue; }\nexport declare class Verify { update(data: Json, encoding?: string): Verify; verify(publicKey: Json, signature: Json, signatureEncoding?: string): boolean; }\nexport declare function createSecretKey(key: Json): KeyObject;\nexport declare function createPrivateKey(key: Json): KeyObject;\nexport declare function createPublicKey(key: Json): KeyObject;\nexport declare function createHash(algorithm: string): Hash;\nexport declare function createHmac(algorithm: string, key: Json): Hmac;\nexport declare function createSign(algorithm: string): Sign;\nexport declare function createVerify(algorithm: string): Verify;\nexport declare function sign(algorithm: string, data: Json, key: Json): JsValue;\nexport declare function verify(algorithm: string, data: Json, key: Json, signature: Json): boolean;\nexport declare function publicEncrypt(key: Json, buffer: Json): JsValue;\nexport declare function privateDecrypt(key: Json, buffer: Json): JsValue;\nexport declare function generateKeyPairSync(type: string, options?: any): any;\nexport declare function generateKeyPair(type: string, options: any, callback: (error: any, publicKey: any, privateKey: any) => void): void;\nexport declare const constants: any;\nexport declare function createCipheriv(algorithm: string, key: Json, iv: Json): Cipheriv;\nexport declare function createDecipheriv(algorithm: string, key: Json, iv: Json): Cipheriv;\nexport declare function pbkdf2Sync(password: Json, salt: Json, iterations: number, keylen: number, digest: string): Buffer;\nexport declare function scrypt(password: Json, salt: Json, keylen: number, options: any, callback: (error: any, value: Buffer) => void): void;\nexport declare function scrypt(password: Json, salt: Json, keylen: number, callback: (error: any, value: Buffer) => void): void;\nexport declare function scryptSync(password: Json, salt: Json, keylen: number, options?: any): Buffer;\nexport declare function randomBytes(size: number): Buffer;\nexport declare function randomFill(argsArray: any): any;\nexport declare function randomFillSync(buffer: Buffer, offset?: number, size?: number): Buffer;\nexport declare function randomInt(max: number): number;\nexport declare function randomInt(min: number, max: number): number;\nexport declare function randomUUID(options?: Json): string;\nexport declare function timingSafeEqual(left: Json, right: Json): boolean;\nexport declare function getHashes(argsArray: any): any;\nexport declare function diffieHellman(options: any): Buffer;\nexport declare class ECDH { constructor(curveName: string); generateKeys(encoding?: string, format?: string): any; getPublicKey(encoding?: string, format?: string): any; getPrivateKey(encoding?: string): any; setPrivateKey(privateKey: Json, encoding?: string): void; computeSecret(otherPublicKey: Json, inputEncoding?: string, outputEncoding?: string): any; }\nexport declare function createECDH(curveName: string): ECDH;\n"
        }
        "perf_hooks" => {
            "export declare const performance: any;\nexport declare class Histogram { readonly min: number; readonly max: number; readonly mean: number; readonly stddev: number; readonly exceeds: number; readonly count: number; readonly percentiles: Map<number, number>; enable(): boolean; disable(): boolean; reset(): void; percentile(value: number): number; percentileBigInt(value: number): bigint; }\nexport declare function monitorEventLoopDelay(options?: { resolution?: number }): Histogram;\nexport declare function createHistogram(options?: any): Histogram;\n"
        }
        "v8" => {
        ...
        // (`data` chunks are strings, like `body`). `bodyHex()` is the
        // binary escape hatch: the raw body bytes as a lowercase hex
        // string (thaw strings are NUL-terminated, so a byte-exact body
        // can't ride `body`/`on("data")` directly), mirroring
        // `response.endEncoded(content, "hex")` outbound. Declared as a
        ...
        // `bodyBytes()` / `endBytes(...)` / `writeBytes(...)` are the
        // first-class `Buffer` (`HirType::Bytes`) surface -- the byte
        // array in and out with no hex detour. Typed as real
        // `Uint8Array` (thaw-bridge's `classify_ts_type` maps it to
        // `HirType::Bytes` while classifying this ambient builtin --
        // see `thaw_bridge::allow_native_bytes_type`'s doc comment for
        // why that mapping is scoped to only ever apply here, never to
        // a real npm package's own `.d.ts`).
        "http" => {
            "export interface IncomingMessage { method: string; url: string; statusCode: number; body: () => string; on: (event: string, callback: (chunk: Uint8Array) => void) => boolean; bodyHex: () => string; bodyBytes: () => Uint8Array; getHeader: (name: string) => string; }\nexport interface ServerResponse { statusCode: number; setHeader: (name: string, value: string) => boolean; end: (chunk: string) => boolean; write: (chunk: string) => boolean; endEncoded: (content: string, encoding: string) => boolean; writeBytes: (chunk: Uint8Array) => boolean; endBytes: (chunk: Uint8Array) => boolean; }\nexport interface Server { listen: (port: number) => string; __listenWithCallback: (port: number, callback: () => void) => string; listenMany: (port: number, count: number) => string; close: () => boolean; __closeWithCallback: (callback: () => void) => boolean; on: (event: string, callback: () => void) => boolean; __onError: (event: string, callback: (error: { message: string; code: string; syscall: string; address: string; port: number }) => void) => boolean; }\nexport declare function serveOnce(port: number, body: string): string;\nexport declare function serveOnceWith(port: number, callback: (target: string) => string): string;\nexport declare function createServerOnce(port: number, callback: (request: IncomingMessage, response: ServerResponse) => void): string;\nexport declare function createServer(callback: (request: IncomingMessage, response: ServerResponse) => void): Server;\nexport declare const get: JsValue;\nexport declare const request: JsValue;\n"
        }
        "https" => {
