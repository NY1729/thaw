...
fn render_bundle(main_key: &str, modules: &[BundledModule]) -> String {
    ...
    out.push_str(
        "function __thaw_bundle_target(map, spec) {\n\
         ...
         \x20\x20\x20\x20\x20\x20if (spec === 'bindings' && globalThis.require && typeof globalThis.require.addon === 'function') return globalThis.require.addon;\n\
         ...
         \x20\x20var created = function(spec) { if (String(spec) === 'bindings' && globalThis.require && typeof globalThis.require.addon === 'function') return globalThis.require.addon; var target = __thaw_bundle_target(map, String(spec)); if (target) return __thaw_bundle_require(target.key, target.factory); return require(String(spec)); };\n\
