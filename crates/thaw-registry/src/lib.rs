    ...
    match name {
        ...
        "tls" => Some(concat!(
            ...
            r#"
             ...
             function rememberCertificates(socket) { if (!socket || !socket._handle) return; var peer = __thaw_tls_peer_certificate(socket._handle), local = __thaw_tls_local_certificate(socket._handle); socket._peerCertificateRaw = peer ? Buffer.from(peer, 'hex') : undefined; socket._localCertificateRaw = local ? Buffer.from(local, 'hex') : undefined; }
             function certificateInfo(raw) { if (!raw) return {}; var digest = __thaw_crypto_hash_hex('sha256', raw.toString('hex')).toUpperCase().replace(/(..)(?=.)/g, '$1:'); return { raw: Buffer.from(raw), fingerprint256: digest, subject: {}, issuer: {}, valid_from: '', valid_to: '', serialNumber: '' }; }
             var connectWithoutCertificates = TLSSocket.prototype.connect; TLSSocket.prototype.connect = function() { var result = connectWithoutCertificates.apply(this, arguments); rememberCertificates(this); return result; }; TLSSocket.prototype.getPeerCertificate = function() { return certificateInfo(this._peerCertificateRaw); }; TLSSocket.prototype.getCertificate = function() { return certificateInfo(this._localCertificateRaw); };
...
mod tests {
    ...
    fn net_server_accepts_and_replies_to_a_real_tcp_client() {
        ...
        assert_eq!(
            result,
            format!(
                r#"[["listening:127.0.0.1:{port}","connection:IPv4","data:ping","close"],false,{port},1,true,true]"#
            )
        );
    ...
    fn tls_client_verifies_custom_ca_and_exchanges_encrypted_bytes() {
        ...
        assert_eq!(
            result,
            r#"[["connect","secureConnect","data:pong","end","close:false"],true,null,true,"h2","TLSv1.3","TLSv1.3",4,4,3,"TLSv1.2",true,true,true]"#
        );
        server.join().unwrap();
        let _ = fs::remove_dir_all(&dir);
        let _ = fs::remove_dir_all(&empty_node_modules);
