...
mod tests {
    ...
    fn worker_threads_load_runtime_computed_absolute_paths() {
        ...
        fs::write(
            &worker_path,
            "var wt = require('node:worker_threads'); wt.parentPort.postMessage([wt.workerData, __filename, __dirname, typeof __thaw_worker_spawn]); wt.parentPort.close();",
        )
        .unwrap();
        fs::write(
            dir.join("index.js"),
        ...
        let result = unsafe { CStr::from_ptr(result) }.to_string_lossy();
        let expected =
            serde_json::json!([[23, path, dir.to_string_lossy(), "function"], 0, "number"])
                .to_string();
