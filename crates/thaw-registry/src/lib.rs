...
"#,
        ...
        "stream" => Some(
            "var defaultByteHighWaterMark = 16384, defaultObjectHighWaterMark = 16; function getDefaultHighWaterMark(objectMode) { return objectMode ? defaultObjectHighWaterMark : defaultByteHighWaterMark; } function setDefaultHighWaterMark(objectMode, value) { value = Number(value); if (!Number.isInteger(value) || value < 0) throw new RangeError('highWaterMark must be a non-negative integer'); if (objectMode) defaultObjectHighWaterMark = value; else defaultByteHighWaterMark = value; } function resolveHighWaterMark(options, objectMode) { var value = options.highWaterMark === undefined ? getDefaultHighWaterMark(objectMode) : Number(options.highWaterMark); if (!Number.isInteger(value) || value < 0) throw new RangeError('highWaterMark must be a non-negative integer'); return value; } function Stream() { this._events = Object.create(null); this.destroyed = false; this.closed = false; this.errored = null; this._readableDidRead = false; this._constructState = 'ready'; this._constructWaiters = []; } function configureConstruct(stream, options) { if (stream._constructConfigured || typeof options.construct !== 'function') return; stream._constructConfigured = true; stream._constructState = 'pending'; stream._construct = options.construct; queueMicrotask(function() { if (stream.destroyed) return; var completed = false; function finish(error) { if (completed) return; completed = true; if (error) { stream._constructState = 'failed'; stream._constructWaiters = []; stream.destroy(error); return; } stream._constructState = 'ready'; var waiters = stream._constructWaiters.splice(0); waiters.forEach(function(waiter) { waiter(); }); } try { stream._construct(finish); } catch (error) { finish(error); } }); } function afterConstruct(stream, action) { if (stream._constructState === 'ready') action(); else if (stream._constructState === 'pending') stream._constructWaiters.push(action); }\n\
             Stream.prototype.on = Stream.prototype.addListener = function(event, listener) { var name = event; (this._events[name] || (this._events[name] = [])).push({ listener: listener, once: false }); if (name === 'data' && this._drainReadable) this.resume(); if (name === 'end' && this.readableEnded) queueMicrotask(function() { listener(); }); return this; };\n\
             Stream.prototype.once = function(event, listener) { var name = event; (this._events[name] || (this._events[name] = [])).push({ listener: listener, once: true }); if (name === 'data' && this._drainReadable) this.resume(); return this; };\n\
             Stream.prototype.prependListener = function(event, listener) { var name = event; (this._events[name] || (this._events[name] = [])).unshift({ listener: listener, once: false }); return this; }; Stream.prototype.prependOnceListener = function(event, listener) { var name = event; (this._events[name] || (this._events[name] = [])).unshift({ listener: listener, once: true }); return this; };\n\
             Stream.prototype.off = Stream.prototype.removeListener = function(event, listener) { var name = event; this._events[name] = (this._events[name] || []).filter(function(entry) { return entry.listener !== listener; }); return this; }; Stream.prototype.removeAllListeners = function(event) { if (arguments.length) delete this._events[event]; else this._events = Object.create(null); return this; }; Stream.prototype.listeners = Stream.prototype.rawListeners = function(event) { return (this._events[event] || []).map(function(entry) { return entry.listener; }); }; Stream.prototype.listenerCount = function(event, listener) { return (this._events[event] || []).filter(function(entry) { return !listener || entry.listener === listener; }).length; }; Stream.prototype.eventNames = function() { return Reflect.ownKeys(this._events).filter(function(name) { return (this._events[name] || []).length > 0; }, this); }; Stream.prototype.setMaxListeners = function(value) { this._maxListeners = Number(value); return this; }; Stream.prototype.getMaxListeners = function() { return this._maxListeners === undefined ? 10 : this._maxListeners; };\
             Stream.prototype.emit = function(event) { var name = event; if (name === 'data') this._readableDidRead = true; var entries = (this._events[name] || []).slice(); var args = Array.prototype.slice.call(arguments, 1); entries.forEach(function(entry) { if (entry.once) this.off(name, entry.listener); entry.listener.apply(this, args); }, this); return entries.length > 0; };\n\
             Stream.prototype.destroy = function(error) { if (this.destroyed) return this; this.destroyed = true; if (this.readable === true && !this.readableEnded) this.readableAborted = true; if (this.writable === true && !this.writableFinished) this.writableAborted = true; this.readable = false; this.writable = false; var self = this, completed = false; function finish(finalError) { if (completed) return; completed = true; self.closed = true; finalError = finalError || error; if (finalError) { self.errored = finalError; self.emit('error', finalError); } self.emit('close'); } if (typeof this._destroy === 'function') { try { this._destroy(error || null, finish); } catch (destroyError) { finish(destroyError); } } else finish(error); return this; };\
             function inherit(child, parent) { child.prototype = Object.create(parent.prototype); child.prototype.constructor = child; }\n\
             function Readable(options) { Stream.call(this); options = options || {}; this.readable = true; this.readableEnded = false; this.readableAborted = false; this.readableFlowing = null; this.readableEncoding = null; this._readableObjectMode = Boolean(options.objectMode || options.readableObjectMode); this.readableObjectMode = this._readableObjectMode; this.readableHighWaterMark = resolveHighWaterMark(options, this._readableObjectMode); this.readableLength = 0; this._paused = false; this._chunks = []; this._read = typeof options.read === 'function' ? options.read : function() {}; this._destroy = typeof options.destroy === 'function' ? options.destroy : null; configureConstruct(this, options); }\
             ...
             function initWritableState(target, options) { options = options || {}; target.writable = true; target.writableEnded = false; target.writableFinished = false; target.writableAborted = false; target._writableObjectMode = Boolean(options.objectMode || options.writableObjectMode); target.writableObjectMode = target._writableObjectMode; target.writableHighWaterMark = resolveHighWaterMark(options, target._writableObjectMode); target.writableLength = 0; target.writableNeedDrain = false; target._writeQueue = []; target._writing = false; target._ending = false; target._finishing = false; target._needsDrain = false; target._constructPumpQueued = false; target._defaultEncoding = options.defaultEncoding || 'utf8'; target._corked = 0; target.writableCorked = 0; target._endCallbacks = []; target._write = typeof options.write === 'function' ? options.write : function(chunk, encoding, callback) { callback(); }; target._writev = typeof options.writev === 'function' ? options.writev : null; target._final = typeof options.final === 'function' ? options.final : null; if (typeof options.destroy === 'function') target._destroy = options.destroy; configureConstruct(target, options); } function Writable(options) { Stream.call(this); initWritableState(this, options); }\n\
             ...
             Writable.prototype.destroySoon = Writable.prototype.end; Writable.prototype[Symbol.asyncDispose] = async function() { if (!this.destroyed && !this.writableFinished) this.destroy(); };\
...
mod tests {
    ...
    fn stream_extended_state_unshift_wrap_and_default_encoding_work() {
        ...
        assert_eq!(
            result,
            r#"["ab",["wrapped"],true,false,true,false,[["hi","hex"]],true,true,true,true,false,false]"#
        );
        let _ = fs::remove_dir_all(&dir);
        let _ = fs::remove_dir_all(&empty_node_modules);
    }
    ...
    fn stream_indexed_pairs_static_checks_and_half_open_control_work() {
        ...
        assert_eq!(
            result,
            r#"[[[0,"a"],[1,"b"]],[false,false,true,false],true,false,true,true,true,true,true]"#
        );
        let _ = fs::remove_dir_all(&dir);
        let _ = fs::remove_dir_all(&empty_node_modules);
    }
    ...
    fn stream_default_high_water_marks_apply_to_new_streams() {
        ...
        .unwrap();
        let empty_node_modules =
            temp_registry("builtin_stream_default_high_water_mark_node_modules");
        let (bundle, _, file_count, _) =
            bundle_commonjs_package(&empty_node_modules, "pkg", &dir, "index.js").unwrap();
        assert_eq!(file_count, 2);
    ...
    fn functional_pipeline_errors_abort_sources_and_destroy_stages() {
        ...
        .unwrap();
        let empty_node_modules =
            temp_registry("builtin_stream_function_pipeline_cleanup_node_modules");
        let (bundle, _, file_count, _) =
            bundle_commonjs_package(&empty_node_modules, "pkg", &dir, "index.js").unwrap();
        assert_eq!(file_count, 3);
    ...
    fn writable_destroy_rejects_pending_work_exactly_once() {
        ...
        assert_eq!(
            result,
            r#"[["write:a","error:boom","close","callback:a:boom","callback:b:boom","end:boom"],0,true,false,["ERR_STREAM_DESTROYED"],true]"#
        );
        let _ = fs::remove_dir_all(&dir);
        let _ = fs::remove_dir_all(&empty_node_modules);
    }
