    ...
    match name {
        ...
        "child_process" => Some(
            r#"var EventEmitter = require('node:events'), streams = require('node:stream');
             ...
             var children = new Map(); function signalName(signal) { return signal === 9 ? 'SIGKILL' : signal === 15 ? 'SIGTERM' : signal === 2 ? 'SIGINT' : signal === null || signal === undefined ? null : 'SIG' + signal; }
             ...
             if (!globalThis.__thaw_child_process_poll_installed) { globalThis.__thaw_child_process_poll_installed = true; var previousPlatformPoll = globalThis.__thaw_poll_platform_events; globalThis.__thaw_poll_platform_events = function() { if (previousPlatformPoll) previousPlatformPoll(); JSON.parse(__thaw_child_process_poll()).forEach(function(event) { var child = children.get(event.handle); if (!child) return; if (event.type === 'spawn') { child.pid = Number(event.pid); child.emit('spawn'); } else if (event.type === 'stdout') { var stdout = Buffer.from(event.value, 'hex'); if (child._stdioModes[1] === 'pipe') child._stdout.write(stdout); else if (child._stdioModes[1] === 'inherit' && process.stdout && process.stdout.write) process.stdout.write(stdout); } else if (event.type === 'stderr') { var stderr = Buffer.from(event.value, 'hex'); if (child._stdioModes[2] === 'pipe') child._stderr.write(stderr); else if (child._stdioModes[2] === 'inherit' && process.stderr && process.stderr.write) process.stderr.write(stderr); } else if (event.type === 'error') { child._spawnError = true; var error = new Error(event.message); error.code = event.code; error.path = child.spawnfile; error.spawnargs = child.spawnargs.slice(1); child.emit('error', error); } else if (event.type === 'exit') { child._closed = true; child.exitCode = event.code === null ? null : Number(event.code); child.signalCode = signalName(event.signal); child._stdout.end(); child._stderr.end(); if (!child._stdin.writableEnded) child._stdin.end(); if (child._timeout) clearTimeout(child._timeout); if (child._abortSignal && child._abortListener) child._abortSignal.removeEventListener('abort', child._abortListener); if (!child._spawnError) child.emit('exit', child.exitCode, child.signalCode); child.emit('close', child.exitCode, child.signalCode); children.delete(event.handle); } }); }; }
             module.exports = { ChildProcess: ChildProcess, spawn: spawn, exec: exec, execFile: execFile, spawnSync: spawnSync, execFileSync: execFileSync, execSync: execSync }; module.exports.default = module.exports; module.exports.__esModule = true;
"#,
        ),
        "punycode" => Some(
...
mod tests {
    ...
    fn child_process_async_apis_stream_and_emit_lifecycle_events() {
        ...
        assert_eq!(
            result,
            r#"[["spawn:true","exit:4:null","close:4:null"],"out:hello","err:hello",4,null,true,true,[null,"callback",""],[6,"failure","","failure"],[null,"SIGKILL",true],["ENOENT",null,null]]"#
        );
        let _ = fs::remove_dir_all(&dir);
        let _ = fs::remove_dir_all(&empty_node_modules);
    }
    ...
    fn child_process_spawn_honors_stdio_timeout_abort_and_detached_options() {
        ...
        assert_eq!(
            result,
            r#"[[null,null,null,[0,null]],[null,null,null,"visible","warning",[0,null]],[true,[null,"SIGKILL"]],[["AbortError","ABORT_ERR"],[null,"SIGKILL"]],[true,[0,null]],"ERR_INVALID_ARG_VALUE"]"#
        );
        let _ = fs::remove_dir_all(&dir);
        let _ = fs::remove_dir_all(&empty_node_modules);
    }
