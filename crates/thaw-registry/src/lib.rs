...
"#,
        ...
        "stream" => Some(
            "var defaultByteHighWaterMark = 16384, defaultObjectHighWaterMark = 16; function getDefaultHighWaterMark(objectMode) { return objectMode ? defaultObjectHighWaterMark : defaultByteHighWaterMark; } function setDefaultHighWaterMark(objectMode, value) { value = Number(value); if (!Number.isInteger(value) || value < 0) throw new RangeError('highWaterMark must be a non-negative integer'); if (objectMode) defaultObjectHighWaterMark = value; else defaultByteHighWaterMark = value; } function resolveHighWaterMark(options, objectMode) { var value = options.highWaterMark === undefined ? getDefaultHighWaterMark(objectMode) : Number(options.highWaterMark); if (!Number.isInteger(value) || value < 0) throw new RangeError('highWaterMark must be a non-negative integer'); return value; } function Stream() { this._events = Object.create(null); this.destroyed = false; this.errored = null; this._readableDidRead = false; }\n\
             ...
             Stream.prototype.destroy = function(error) { if (this.destroyed) return this; this.destroyed = true; if (error) { this.errored = error; this.emit('error', error); } this.emit('close'); return this; };\n\
             function inherit(child, parent) { child.prototype = Object.create(parent.prototype); child.prototype.constructor = child; }\n\
             function Readable(options) { Stream.call(this); options = options || {}; this.readable = true; this.readableEnded = false; this.readableEncoding = null; this._readableObjectMode = Boolean(options.objectMode || options.readableObjectMode); this.readableHighWaterMark = resolveHighWaterMark(options, this._readableObjectMode); this.readableLength = 0; this._paused = false; this._chunks = []; this._read = typeof options.read === 'function' ? options.read : function() {}; }\
             ...
             function initWritableState(target, options) { options = options || {}; target.writable = true; target.writableEnded = false; target.writableFinished = false; target._writableObjectMode = Boolean(options.objectMode || options.writableObjectMode); target.writableHighWaterMark = resolveHighWaterMark(options, target._writableObjectMode); target.writableLength = 0; target._writeQueue = []; target._writing = false; target._ending = false; target._finishing = false; target._needsDrain = false; target._corked = 0; target.writableCorked = 0; target._endCallbacks = []; target._write = typeof options.write === 'function' ? options.write : function(chunk, encoding, callback) { callback(); }; target._writev = typeof options.writev === 'function' ? options.writev : null; target._final = typeof options.final === 'function' ? options.final : null; } function Writable(options) { Stream.call(this); initWritableState(this, options); }\n\
             inherit(Writable, Stream);\n\
             Writable.prototype._pumpWrites = function() { var self = this; if (this._writing || this._corked) return; if (!this._writeQueue.length) { if (this._ending) this._finishWrites(); return; } this._writing = true; var entries = this._writev && this._writeQueue.length > 1 ? this._writeQueue.splice(0) : [this._writeQueue.shift()], total = entries.reduce(function(sum, entry) { return sum + entry.length; }, 0), complete = function(error) { self._writing = false; self.writableLength -= total; var finalError = error || self._destroyError; entries.forEach(function(entry) { if (entry.callback) entry.callback(finalError); }); if (self.destroyed) return; if (error) { self.destroy(error); return; } if (self._needsDrain && self.writableLength < self.writableHighWaterMark) { self._needsDrain = false; self.emit('drain'); } self._pumpWrites(); }; if (this._writev && entries.length > 1) this._writev(entries.map(function(entry) { return { chunk: entry.chunk, encoding: entry.encoding }; }), complete); else this._write(entries[0].chunk, entries[0].encoding, complete); }; Writable.prototype._finishWrites = function() { if (this._finishing || this.writableFinished) return; this._finishing = true; var self = this, finish = function(error) { if (error) { self.destroy(error); return; } self.writable = false; self.writableFinished = true; self.emit('finish'); self._endCallbacks.splice(0).forEach(function(callback) { callback(); }); }; if (this._final) this._final(finish); else finish(); }; Writable.prototype.write = function(chunk, encoding, callback) { if (typeof encoding === 'function') { callback = encoding; encoding = undefined; } if (this.writableEnded) { var error = new Error('write after end'); error.code = 'ERR_STREAM_WRITE_AFTER_END'; throw error; } var value = this._writableObjectMode ? chunk : typeof chunk === 'string' ? globalThis.Buffer.from(chunk, encoding) : globalThis.Buffer.from(chunk), length = this._writableObjectMode ? 1 : value.length; this._writeQueue.push({ chunk: value, encoding: this._writableObjectMode ? 'utf8' : encoding || 'buffer', callback: callback, length: length }); this.writableLength += length; var available = this.writableLength < this.writableHighWaterMark; if (!available) this._needsDrain = true; this._pumpWrites(); return available; };\n\
             Writable.prototype.end = function(chunk, encoding, callback) { if (typeof chunk === 'function') { callback = chunk; chunk = undefined; } else if (typeof encoding === 'function') { callback = encoding; encoding = undefined; } if (chunk !== undefined) this.write(chunk, encoding); if (callback) this._endCallbacks.push(callback); this.writableEnded = true; this._ending = true; this._corked = 0; this.writableCorked = 0; this._pumpWrites(); return this; }; Writable.prototype.destroy = function(error) { if (this.destroyed) return this; var callbackError = error || new Error('Cannot call write after a stream was destroyed'); if (!callbackError.code) callbackError.code = 'ERR_STREAM_DESTROYED'; this._destroyError = callbackError; this.writable = false; this._ending = false; var pending = this._writeQueue.splice(0), self = this; this.writableLength -= pending.reduce(function(total, entry) { return total + entry.length; }, 0); queueMicrotask(function() { pending.forEach(function(entry) { if (entry.callback) entry.callback(callbackError); }); self._endCallbacks.splice(0).forEach(function(callback) { callback(callbackError); }); }); return Stream.prototype.destroy.call(this, error); }; Writable.prototype.cork = function() { this._corked++; this.writableCorked = this._corked; }; Writable.prototype.uncork = function() { if (this._corked > 0) this._corked--; this.writableCorked = this._corked; if (!this._corked) this._pumpWrites(); };\
