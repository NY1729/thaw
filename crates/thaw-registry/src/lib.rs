...
"#,
        ...
        "worker_threads" => Some(concat!(
            ...
            r#"
             ...
             var nativeWorkers = globalThis.__thaw_native_workers || (globalThis.__thaw_native_workers = new Map()); function nativeTransferListSupported(transfer) { return Array.from(transfer || []).every(function(value) { return value instanceof ArrayBuffer; }); } function canUseNativeWorker(source, options) { if (typeof __thaw_worker_spawn !== 'function' || options.stdin || options.stdout || options.stderr || options.env === SHARE_ENV || !nativeTransferListSupported(options.transferList)) return false; var text = String(source); if (text.indexOf('postMessageToThread') >= 0 || text.indexOf('workerMessage') >= 0) return false; try { __thaw_worker_encode(options.workerData === undefined ? null : options.workerData); return true; } catch (error) { return false; } } function detachNativeTransfers(transfer) { assertTransferList(transfer); for (var value of transfer || []) __thaw_detach_array_buffer(value); } function startNativeWorker(worker, source, options) { if (!canUseNativeWorker(source, options)) return false; worker.stdin = worker.stdout = worker.stderr = null; worker._terminated = false; worker._exited = false; var parentProcess = globalThis.process || {}, config = { threadName: options.name === undefined ? '' : String(options.name), resourceLimits: Object.assign({}, options.resourceLimits || {}), env: Object.assign({}, options.env === undefined ? (parentProcess.env || {}) : options.env), argv: Array.from(parentProcess.argv || []).concat(Array.from(options.argv || [], String)), execArgv: options.execArgv === undefined ? Array.from(parentProcess.execArgv || []) : Array.from(options.execArgv || [], String) }, workerDataPayload = __thaw_worker_encode(options.workerData === undefined ? null : options.workerData); detachNativeTransfers(options.transferList); worker._nativeHandle = __thaw_worker_spawn(String(globalThis.__thaw_worker_bundle_source || ''), String(source), workerDataPayload, JSON.stringify(config), worker.threadId); nativeWorkers.set(worker._nativeHandle, worker); return true; } if (!globalThis.__thaw_native_worker_poll_installed) { globalThis.__thaw_native_worker_poll_installed = true; var previousPlatformPoll = globalThis.__thaw_poll_platform_events; globalThis.__thaw_poll_platform_events = function() { if (previousPlatformPoll) previousPlatformPoll(); var events = JSON.parse(__thaw_worker_poll()); events.forEach(function(event) { var worker = nativeWorkers.get(event.handle); if (!worker) return; if (event.type === 'online') worker.emit('online'); else if (event.type === 'message') worker.emit('message', __thaw_worker_decode(event.payload)); else if (event.type === 'error') worker.emit('error', new Error(event.error)); else if (event.type === 'exit') { nativeWorkers.delete(event.handle); worker._finish(event.code); } }); }; }
