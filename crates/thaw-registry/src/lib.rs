...
mod tests {
    ...
    fn worker_threads_load_runtime_computed_absolute_paths() {
        ...
        fs::write(
            dir.join("package.json"),
            r##"{"imports":{"#worker-tool":{"node":"./internal/tool.cjs","default":"./internal/wrong.cjs"},"#features/*":{"require":"./internal/features/*.js"}}}"##,
        )
        .unwrap();
        ...
        fs::write(
            ...
            r#"{"main":"wrong.cjs","exports":{".":{"node":"./entry.cjs","default":"./wrong.cjs"},"./feature":{"require":"./feature.js"},"./features/*":{"require":"./dist/*.cjs"}}}"#,
        )
        .unwrap();
        fs::write(
        ...
        fs::write(
            &worker_path,
            "var wt = require('node:worker_threads'); var hiddenCode; try { require('runtime-worker-dependency/hidden'); } catch (error) { hiddenCode = error.code; } var missingImportCode; try { require('#missing'); } catch (error) { missingImportCode = error.code; } wt.parentPort.postMessage([wt.workerData, __filename, __dirname, typeof __thaw_worker_spawn, require('./worker-value'), require('runtime-worker-dependency'), require('runtime-worker-dependency/feature'), require('runtime-worker-dependency/features/math'), hiddenCode, require('#worker-tool'), require('#features/math'), missingImportCode]); wt.parentPort.close();",
        )
        .unwrap();
        fs::write(
            dir.join("index.js"),
