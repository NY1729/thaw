...
mod tests {
    ...
    #[test]
    fn streams_throw_unhandled_errors_and_still_close_on_destroy() {
        use std::ffi::{CStr, CString};
        let dir = temp_registry("builtin_stream_unhandled_errors");
        fs::write(
            dir.join("index.js"),
            "var stream = require('node:stream'); module.exports = function () { var expected = new Error('expected'), direct, contextError; try { new stream.Stream().emit('error', expected); } catch (error) { direct = error === expected; } var context = { value: 1 }; try { new stream.Stream().emit('error', context); } catch (error) { contextError = [error.code, error.context === context]; } var handledValue, handled = new stream.Stream(); handled.on('error', function(error) { handledValue = error; }); var handledResult = handled.emit('error', expected); var closeCount = 0, destroyed = new stream.Readable(); destroyed.on('close', function() { closeCount++; }); var destroyThrown; try { destroyed.destroy(expected); } catch (error) { destroyThrown = error === expected; } var hookThrown, hooked = new stream.Readable({ destroy: function(error, callback) { callback(error); } }); hooked.on('close', function() { closeCount++; }); try { hooked.destroy(expected); } catch (error) { hookThrown = error === expected; } return [direct, contextError, handledResult, handledValue === expected, destroyThrown, destroyed.closed, destroyed.errored === expected, hookThrown, hooked.closed, closeCount]; };",
        )
        .unwrap();
        ...
        assert_eq!(
            result,
            r#"[true,["ERR_UNHANDLED_ERROR",true],true,true,true,true,true,true,true,2]"#
        );
        let _ = fs::remove_dir_all(&dir);
        let _ = fs::remove_dir_all(&empty_node_modules);
    }
