...
"#,
        ...
        "worker_threads" => Some(concat!(
            ...
            r#"
             ...
             function exitCompletion(worker) { if (!worker._exitPromise) worker._exitPromise = new Promise(function(resolve) { worker._resolveExit = resolve; }); return worker._exitPromise; } var finishWithoutCompletion = Worker.prototype._finish; Worker.prototype._finish = function(code) { if (this._exited) return exitCompletion(this); var completion = exitCompletion(this), worker = this, exitCode = Number(code); finishWithoutCompletion.call(this, exitCode); queueMicrotask(function() { worker._resolveExit(exitCode); }); return completion; }; Worker.prototype.terminate = function() { var completion = exitCompletion(this); if (!this._terminated && !this._exited) { this._terminated = true; this._finish(1); this._workerPort.close(); this._port.close(); } return completion; }; if (Symbol.asyncDispose) Worker.prototype[Symbol.asyncDispose] = Worker.prototype.terminate;
...
mod tests {
    ...
    fn worker_threads_builtin_exchanges_cloned_messages() {
        ...
        assert_eq!(
            result,
            r#"[true,0,null,7,42,true,false,true,true,true,true,true,true]"#
        );
        let _ = fs::remove_dir_all(&dir);
        let _ = fs::remove_dir_all(&empty_node_modules);
    }
