...
"#,
        ...
        "fs" => Some(
            "var stream = require('node:stream'); function pathValue(path) { return path instanceof URL ? decodeURIComponent(path.pathname) : String(path); } function invoke(operation, path, value, recursive) { path = pathValue(path); var record = JSON.parse(globalThis.__thaw_fs(operation, path, value || '', Boolean(recursive))); if (!record.ok) { var error = new Error(record.code + ': ' + record.message + ', ' + operation + ' ' + path); error.code = record.code; error.errno = -1; error.path = path; error.syscall = operation; throw error; } return record; } function encoding(options) { return typeof options === 'string' ? options : options && options.encoding; } function data(value, options) { return Buffer.isBuffer(value) || value instanceof Uint8Array ? Buffer.from(value) : Buffer.from(String(value), encoding(options)); } function timeValue(value) { if (value instanceof Date) return value.getTime() / 1000; var number = Number(value); if (Number.isFinite(number)) return number; return new Date(value).getTime() / 1000; } function Stats(record) { this.size = Number(record.length); this.mode = Number(record.mode); this.atimeMs = Number(record.atimeMs); this.mtimeMs = Number(record.mtimeMs); this.ctimeMs = Number(record.ctimeMs); this.birthtimeMs = Number(record.birthtimeMs); this.atime = new Date(this.atimeMs); this.mtime = new Date(this.mtimeMs); this.ctime = new Date(this.ctimeMs); this.birthtime = new Date(this.birthtimeMs); this._file = record.file; this._directory = record.directory; } Stats.prototype.isFile = function() { return this._file; }; Stats.prototype.isDirectory = function() { return this._directory; }; Stats.prototype.isSymbolicLink = Stats.prototype.isSocket = Stats.prototype.isFIFO = Stats.prototype.isCharacterDevice = Stats.prototype.isBlockDevice = function() { return false; };\n\
             ...
             __thaw_fs.linkSync = function(existingPath, newPath) { invoke('link', existingPath, pathValue(newPath)); }; __thaw_fs.symlinkSync = function(target, path) { invoke('symlink', target, pathValue(path)); }; __thaw_fs.readlinkSync = function(path, options) { var result = invoke('readlink', path).path; return encoding(options) === 'buffer' ? Buffer.from(result) : result; }; __thaw_fs.chmodSync = function(path, mode) { if (typeof mode === 'string') mode = parseInt(mode, 8); invoke('chmod', path, String(Number(mode))); }; __thaw_fs.utimesSync = function(path, atime, mtime) { invoke('utimes', path, String(timeValue(atime)) + ',' + String(timeValue(mtime))); };\n\
             ...
             FileHandle.prototype.utimes = function(atime, mtime) { var self = this; return Promise.resolve().then(function() { self._check(); __thaw_fs.utimesSync(self.path, atime, mtime); }); }; Object.assign(__thaw_fs.promises, { link: promised(__thaw_fs.linkSync), symlink: promised(__thaw_fs.symlinkSync), readlink: promised(__thaw_fs.readlinkSync), chmod: promised(__thaw_fs.chmodSync), utimes: promised(__thaw_fs.utimesSync) }); __thaw_fs.link = callbackMethod(__thaw_fs.linkSync, false); __thaw_fs.symlink = callbackMethod(__thaw_fs.symlinkSync, false); __thaw_fs.readlink = callbackMethod(__thaw_fs.readlinkSync, true); __thaw_fs.chmod = callbackMethod(__thaw_fs.chmodSync, false); __thaw_fs.utimes = callbackMethod(__thaw_fs.utimesSync, false);\n\
