...
mod tests {
    ...
    fn worker_threads_eval_worker_isolates_state_and_exchanges_messages() {
        ...
        assert_eq!(
            result,
            r#"[["online","message:42:false:true:alpha:99:one,2:--trace-warnings:child","exit:0"],"alpha",["first","regular","regular"],2,true,0,20,"undefined",null,"shared",64,true,true,true,1]"#
        );
        let _ = fs::remove_dir_all(&dir);
        let _ = fs::remove_dir_all(&empty_node_modules);
    }
