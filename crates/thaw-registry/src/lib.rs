    ...
    match name {
        ...
        "child_process" => Some(
            r#"var EventEmitter = require('node:events'), streams = require('node:stream');
             ...
             function ChildProcess(normalized) { EventEmitter.call(this); this.pid = 0; this.spawnfile = normalized.command; this.spawnargs = [normalized.command].concat(normalized.args); this.killed = false; this.connected = false; this.exitCode = null; this.signalCode = null; this.stdin = new streams.Writable({ write: function(chunk, encoding, callback) { if (!child._closed) __thaw_child_process_stdin(child._handle, Buffer.from(chunk).toString('hex'), false); callback(); }, final: function(callback) { if (!child._closed) __thaw_child_process_stdin(child._handle, '', true); callback(); } }); this.stdout = new streams.PassThrough(); this.stderr = new streams.PassThrough(); this.stdio = [this.stdin, this.stdout, this.stderr]; this.channel = null; this._closed = false; this._spawnError = false; var child = this; this._handle = __thaw_child_process_spawn(normalized.command, JSON.stringify(normalized.args), JSON.stringify(normalized.options)); children.set(this._handle, this); }
