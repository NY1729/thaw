...
mod tests {
    ...
    fn tls_client_verifies_custom_ca_and_exchanges_encrypted_bytes() {
        ...
        assert_eq!(
            result,
            r#"[["connect","secureConnect","data:pong","end","close:false"],true,null,true,"h2","TLSv1.3","TLSv1.3",4,4,3,"TLSv1.2",true,true,true,"localhost","localhost","thaw-client",true,true,true]"#
        );
        server.join().unwrap();
        let _ = fs::remove_dir_all(&dir);
        let _ = fs::remove_dir_all(&empty_node_modules);
