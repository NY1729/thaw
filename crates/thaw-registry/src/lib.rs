...
mod tests {
    ...
    fn child_process_fork_exchanges_ipc_messages_and_disconnects() {
        ...
        assert_eq!(
            result,
            r#"[["spawn","message:42:argument","disconnect","close:0:null"],"child-ready",null,false,true,"ERR_IPC_CHANNEL_CLOSED",true]"#
        );
        let _ = fs::remove_dir_all(&dir);
        let _ = fs::remove_dir_all(&empty_node_modules);
    }
