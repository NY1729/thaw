...
mod tests {
    ...
    fn fs_write_streams_apply_backpressure_and_serialize_callbacks() {
        ...
        assert_eq!(
            result,
            r#"[true,false,"ab",0,3,["first","second","drain","end","finish"],"abcdef",false,1,"x"]"#
        );
        let _ = fs::remove_dir_all(&dir);
        let _ = fs::remove_dir_all(&empty_node_modules);
    }
    ...
    fn stream_writables_serialize_async_work_and_signal_backpressure() {
        ...
        assert_eq!(
            result,
            r#"[true,false,0,true,["start:a","end:a","callback:a","start:bb","end:bb","callback:bb","drain","start:c","end:c","end-callback","finish"],true,false,"XYZ",true]"#
        );
        let _ = fs::remove_dir_all(&dir);
        let _ = fs::remove_dir_all(&empty_node_modules);
    }
