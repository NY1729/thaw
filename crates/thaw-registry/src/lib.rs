...
fn builtin_module_source(name: &str) -> Option<&'static str> {
    match name {
        ...
        "module" => Some(
            "var builtinModules = ['assert','assert/strict','async_hooks','buffer','console','constants','crypto','diagnostics_channel','events','fs','fs/promises','http','module','os','path','perf_hooks','process','querystring','readline','readline/promises','stream','stream/promises','string_decoder','timers','timers/promises','tty','url','util','util/types','v8','worker_threads','zlib']; var builtinSet = new Set(builtinModules);\n\
