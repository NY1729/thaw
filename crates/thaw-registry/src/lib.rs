...
"#,
        ...
        "stream" => Some(
            "function Stream() { this._events = Object.create(null); this.destroyed = false; this.errored = null; this._readableDidRead = false; }\n\
             ...
             Writable.prototype._pumpWrites = function() { var self = this; if (this._writing || this._corked) return; if (!this._writeQueue.length) { if (this._ending) this._finishWrites(); return; } this._writing = true; var entries = this._writev && this._writeQueue.length > 1 ? this._writeQueue.splice(0) : [this._writeQueue.shift()], total = entries.reduce(function(sum, entry) { return sum + entry.chunk.length; }, 0), complete = function(error) { self._writing = false; self.writableLength -= total; var finalError = error || self._destroyError; entries.forEach(function(entry) { if (entry.callback) entry.callback(finalError); }); if (self.destroyed) return; if (error) { self.destroy(error); return; } if (self._needsDrain && self.writableLength < self.writableHighWaterMark) { self._needsDrain = false; self.emit('drain'); } self._pumpWrites(); }; if (this._writev && entries.length > 1) this._writev(entries.map(function(entry) { return { chunk: entry.chunk, encoding: entry.encoding }; }), complete); else this._write(entries[0].chunk, entries[0].encoding, complete); }; Writable.prototype._finishWrites = function() { if (this._finishing || this.writableFinished) return; this._finishing = true; var self = this, finish = function(error) { if (error) { self.destroy(error); return; } self.writable = false; self.writableFinished = true; self.emit('finish'); self._endCallbacks.splice(0).forEach(function(callback) { callback(); }); }; if (this._final) this._final(finish); else finish(); }; Writable.prototype.write = function(chunk, encoding, callback) { if (typeof encoding === 'function') { callback = encoding; encoding = undefined; } if (this.writableEnded) { var error = new Error('write after end'); error.code = 'ERR_STREAM_WRITE_AFTER_END'; throw error; } var value = typeof chunk === 'string' ? globalThis.Buffer.from(chunk, encoding) : globalThis.Buffer.from(chunk); this._writeQueue.push({ chunk: value, encoding: encoding || 'buffer', callback: callback }); this.writableLength += value.length; var available = this.writableLength < this.writableHighWaterMark; if (!available) this._needsDrain = true; this._pumpWrites(); return available; };\n\
             Writable.prototype.end = function(chunk, encoding, callback) { if (typeof chunk === 'function') { callback = chunk; chunk = undefined; } else if (typeof encoding === 'function') { callback = encoding; encoding = undefined; } if (chunk !== undefined) this.write(chunk, encoding); if (callback) this._endCallbacks.push(callback); this.writableEnded = true; this._ending = true; this._corked = 0; this.writableCorked = 0; this._pumpWrites(); return this; }; Writable.prototype.destroy = function(error) { if (this.destroyed) return this; var callbackError = error || new Error('Cannot call write after a stream was destroyed'); if (!callbackError.code) callbackError.code = 'ERR_STREAM_DESTROYED'; this._destroyError = callbackError; this.writable = false; this._ending = false; var pending = this._writeQueue.splice(0), self = this; this.writableLength -= pending.reduce(function(total, entry) { return total + entry.chunk.length; }, 0); queueMicrotask(function() { pending.forEach(function(entry) { if (entry.callback) entry.callback(callbackError); }); self._endCallbacks.splice(0).forEach(function(callback) { callback(callbackError); }); }); return Stream.prototype.destroy.call(this, error); }; Writable.prototype.cork = function() { this._corked++; this.writableCorked = this._corked; }; Writable.prototype.uncork = function() { if (this._corked > 0) this._corked--; this.writableCorked = this._corked; if (!this._corked) this._pumpWrites(); };\
