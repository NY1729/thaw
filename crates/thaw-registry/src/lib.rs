...
"#,
        ...
        "stream" => Some(
            "var defaultByteHighWaterMark = 16384, defaultObjectHighWaterMark = 16; function getDefaultHighWaterMark(objectMode) { return objectMode ? defaultObjectHighWaterMark : defaultByteHighWaterMark; } function setDefaultHighWaterMark(objectMode, value) { value = Number(value); if (!Number.isInteger(value) || value < 0) throw new RangeError('highWaterMark must be a non-negative integer'); if (objectMode) defaultObjectHighWaterMark = value; else defaultByteHighWaterMark = value; } function resolveHighWaterMark(options, objectMode) { var value = options.highWaterMark === undefined ? getDefaultHighWaterMark(objectMode) : Number(options.highWaterMark); if (!Number.isInteger(value) || value < 0) throw new RangeError('highWaterMark must be a non-negative integer'); return value; } function Stream() { this._events = Object.create(null); this.destroyed = false; this.closed = false; this.errored = null; this._readableDidRead = false; }\n\
             Stream.prototype.on = function(event, listener) { var name = String(event); (this._events[name] || (this._events[name] = [])).push({ listener: listener, once: false }); if (name === 'data' && this._drainReadable) { this.readableFlowing = true; this._paused = false; this._drainReadable(); } if (name === 'end' && this.readableEnded) queueMicrotask(function() { listener(); }); return this; };\n\
             Stream.prototype.once = function(event, listener) { var name = String(event); (this._events[name] || (this._events[name] = [])).push({ listener: listener, once: true }); if (name === 'data' && this._drainReadable) { this.readableFlowing = true; this._paused = false; this._drainReadable(); } return this; };\n\
             ...
             Stream.prototype.emit = function(event) { var name = String(event); if (name === 'data') this._readableDidRead = true; var entries = (this._events[name] || []).slice(); var args = Array.prototype.slice.call(arguments, 1); entries.forEach(function(entry) { if (entry.once) this.off(name, entry.listener); entry.listener.apply(this, args); }, this); return entries.length > 0; };\n\
             Stream.prototype.destroy = function(error) { if (this.destroyed) return this; this.destroyed = true; this.closed = true; if (this.readable === true && !this.readableEnded) this.readableAborted = true; if (this.writable === true && !this.writableFinished) this.writableAborted = true; this.readable = false; this.writable = false; if (error) { this.errored = error; this.emit('error', error); } this.emit('close'); return this; };\n\
             function inherit(child, parent) { child.prototype = Object.create(parent.prototype); child.prototype.constructor = child; }\n\
             function Readable(options) { Stream.call(this); options = options || {}; this.readable = true; this.readableEnded = false; this.readableAborted = false; this.readableFlowing = null; this.readableEncoding = null; this._readableObjectMode = Boolean(options.objectMode || options.readableObjectMode); this.readableObjectMode = this._readableObjectMode; this.readableHighWaterMark = resolveHighWaterMark(options, this._readableObjectMode); this.readableLength = 0; this._paused = false; this._chunks = []; this._read = typeof options.read === 'function' ? options.read : function() {}; }\
        ...
        "stream/promises" => Some(
            "function streamAbortError(signal) { var error = new DOMException('The operation was aborted', 'AbortError'); error.cause = signal.reason; return error; } function finished(stream, options) { options = options || {}; return new Promise(function(resolve, reject) { var settled = false, abort; function cleanup() { stream.removeListener('error', done); stream.removeListener('finish', complete); stream.removeListener('end', complete); if (options.signal && abort) options.signal.removeEventListener('abort', abort); } function done(error) { if (settled) return; settled = true; cleanup(); if (error) reject(error); else resolve(); } function complete() { done(); } stream.once('error', done); if (stream.writable !== undefined) stream.once('finish', complete); else stream.once('end', complete); abort = function() { done(streamAbortError(options.signal)); }; if (options.signal) { if (options.signal.aborted) abort(); else options.signal.addEventListener('abort', abort, { once: true }); } }); }\
            function pipeline() { var streams = Array.prototype.slice.call(arguments), options = streams.length && streams[streams.length - 1] && typeof streams[streams.length - 1].on !== 'function' && (streams[streams.length - 1].signal !== undefined || streams[streams.length - 1].end !== undefined) ? streams.pop() : {}; return new Promise(function(resolve, reject) { var settled = false, listeners = [], abort; function cleanup() { listeners.forEach(function(entry) { entry.stream.removeListener(entry.event, entry.listener); }); if (options.signal && abort) options.signal.removeEventListener('abort', abort); } function finish(error) { if (settled) return; settled = true; cleanup(); if (error) { streams.forEach(function(item) { if (item.destroy) item.destroy(error); }); reject(error); } else resolve(streams[streams.length - 1]); } streams.forEach(function(item) { var listener = function(error) { finish(error); }; item.once('error', listener); listeners.push({ stream: item, event: 'error', listener: listener }); }); for (var index = 0; index + 1 < streams.length; index++) streams[index].pipe(streams[index + 1]); var last = streams[streams.length - 1], complete = function() { finish(); }; last.once('finish', complete); listeners.push({ stream: last, event: 'finish', listener: complete }); abort = function() { finish(streamAbortError(options.signal)); }; if (options.signal) { if (options.signal.aborted) abort(); else options.signal.addEventListener('abort', abort, { once: true }); } }); }\
