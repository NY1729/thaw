...
"#,
        ...
        "worker_threads" => Some(concat!(
            ...
            r#"
             ...
             var nativeWorkers = globalThis.__thaw_native_workers || (globalThis.__thaw_native_workers = new Map()); function canUseNativeWorker(source, options) { if (typeof __thaw_worker_spawn !== 'function' || options.stdin || options.stdout || options.stderr || options.env === SHARE_ENV || (options.transferList && options.transferList.length) || options.argv || options.execArgv || options.name || options.resourceLimits) return false; var text = String(source); if (text.indexOf('postMessageToThread') >= 0 || text.indexOf('workerMessage') >= 0 || text.indexOf('process.') >= 0) return false; try { return JSON.stringify(options.workerData === undefined ? null : options.workerData) !== undefined; } catch (error) { return false; } } function startNativeWorker(worker, source, options) { if (!canUseNativeWorker(source, options)) return false; worker.stdin = worker.stdout = worker.stderr = null; worker._terminated = false; worker._exited = false; worker._nativeHandle = __thaw_worker_spawn(String(globalThis.__thaw_worker_bundle_source || ''), String(source), JSON.stringify(options.workerData === undefined ? null : options.workerData), worker.threadId); nativeWorkers.set(worker._nativeHandle, worker); return true; } if (!globalThis.__thaw_native_worker_poll_installed) { globalThis.__thaw_native_worker_poll_installed = true; var previousPlatformPoll = globalThis.__thaw_poll_platform_events; globalThis.__thaw_poll_platform_events = function() { if (previousPlatformPoll) previousPlatformPoll(); var events = JSON.parse(__thaw_worker_poll()); events.forEach(function(event) { var worker = nativeWorkers.get(event.handle); if (!worker) return; if (event.type === 'online') worker.emit('online'); else if (event.type === 'message') worker.emit('message', event.value); else if (event.type === 'error') worker.emit('error', new Error(event.error)); else if (event.type === 'exit') { nativeWorkers.delete(event.handle); worker._finish(event.code); } }); }; }
...
mod tests {
    ...
    fn worker_threads_eval_worker_isolates_state_and_exchanges_messages() {
        ...
        assert_eq!(
            result,
            r#"[["online","message:42:false:true:alpha:99:one,2:--trace-warnings:child","exit:0"],"alpha",["first","regular","regular"],2,true,0,20,"undefined",null,"shared",64,true,true,true,0]"#
        );
        let _ = fs::remove_dir_all(&dir);
        let _ = fs::remove_dir_all(&empty_node_modules);
    }
    ...
    fn worker_threads_transfers_worker_data_message_ports() {
        ...
        fs::write(
            dir.join("index.js"),
            "var workers = require('node:worker_threads'); module.exports = async function () { var channel = new workers.MessageChannel(); var source = \"var wt = require('node:worker_threads'); wt.workerData.port.postMessage('from-worker'); wt.workerData.port.close(); wt.parentPort.close();\"; var worker = new workers.Worker(source, { eval: true, workerData: { port: channel.port1 }, transferList: [channel.port1] }); var message = new Promise(function(resolve) { channel.port2.once('message', resolve); }); var detached = channel.port1.__thawClosed && channel.port1.__thawPeer === null; var exit = new Promise(function(resolve, reject) { worker.on('error', reject); worker.on('exit', resolve); }); return [await message, await exit, detached]; };",
        )
        .unwrap();
        ...
        let result = thaw_quickjs::thaw_js_call(function.as_ptr(), arguments.as_ptr());
        let result = unsafe { CStr::from_ptr(result) }.to_string_lossy();
        assert_eq!(result, r#"["from-worker",0,true]"#);
        let _ = fs::remove_dir_all(dir);
        let _ = fs::remove_dir_all(node_modules);
    }
    ...
    fn worker_threads_routes_messages_by_thread_id() {
        ...
        assert_eq!(
            result,
            r#"[[["hello",1]],["direct:ping",0],0,"ERR_WORKER_MESSAGING_SAME_THREAD","ERR_WORKER_MESSAGING_FAILED","ERR_WORKER_MESSAGING_FAILED",["ERR_WORKER_MESSAGING_ERRORED","listener failed"]]"#
        );
        let _ = fs::remove_dir_all(dir);
        let _ = fs::remove_dir_all(node_modules);
    }
