    ...
    fn lower_native_scalar_method(
        ...
        call: &CallExpr,
                ...
                if property.sym == *"toExponential" {
                    ...
                    if arguments.len() > 1 {
                        return Err("native `.toExponential()` expects zero or one argument".into());
                    }
                    let receiver_name =
                        format!("__thaw_to_exponential_receiver_{}", self.next_binding);
                    self.next_binding += 1;
                    self.scope.insert(receiver_name.clone(), HirType::F64);
                    let var = |name: &str| HirExpr::Var(name.into());
                    // Omitted argument: the runtime's `-1` sentinel selects
                    // the shortest round-trippable representation, exactly
                    // like `Number.prototype.toExponential()` with no
                    // argument.
                    let Some(argument) = arguments.first() else {
                        let mut bindings = vec![(receiver_name.clone(), HirType::F64, receiver)];
                        bindings.extend(spread_bindings);
                    ...
                    self.next_binding += 1;
                    let normalized_name =
                        format!("__thaw_to_exponential_normalized_{}", self.next_binding);
                    self.next_binding += 1;
                    self.scope.insert(digits_name.clone(), HirType::F64);
                    self.scope.insert(normalized_name.clone(), HirType::F64);
                    let number = |value| HirExpr::Lit(HirLit::F64(value));
                    ...
                    let range_error = || {
                        HirStmt::Throw(HirExpr::Lit(HirLit::Str(
                            "toExponential() argument must be between 0 and 100".into(),
                        )))
                    };
                    let body = HirExpr::Block(vec![
                        HirStmt::Let(normalized_name.clone(), HirType::F64, var(&digits_name)),
