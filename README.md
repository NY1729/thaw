...
- Ambient and `.d.ts` Fast-path declarations ending in number, boolean or
  string array rest parameters lower to real C varargs. Extras are passed as
  `double`, C-promoted `int`, or NUL-terminated `const char *`, respectively
...
- A fully general ABI-description format. Version 4 covers string layouts,
  ...
  boolean and signed/unsigned integer bitfields, plus explicit direct register
  classes for aggregates up to 16 bytes. Other vararg types are not yet described
