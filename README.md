...
- Heterogeneous native unions use an explicit tag and word payload across
  ...
  Number, string and boolean literal comparisons narrow the matching branch;
  their non-matching branch conservatively retains that primitive member
  because another value of the same native type may still be present
...
- Top-level object and array destructuring supports nesting, defaults, rest
  bindings and runtime-computed dictionary keys, including computed-key exclusion
...
- Ambient declarations and C ABI calls; number-, string-, boolean-, and `JsValue`-array parameters
  ...
  Thaw arena. Borrowed returns for all four array types use the same portable
  `{ data, length }` shape; boolean arrays use one-byte C elements. Fixed
  optional/nullable parameters expand to a one-byte present tag plus payload;
  nullish parameters use a one-byte three-state tag plus payload, recursively
  inside fixed objects. Metadata-selected portable tagged returns use the
  corresponding C struct and recursively restore their payload. Typed tuples
  flatten to declaration-ordered fixed arguments and use portable C structs on return
...
- Automatically generated registry JavaScript bundles are stored as gzip and
  decompressed only when selected; existing plain `bundle.js` entries remain
  compatible
...
- The native compilation path also supports `structuredClone` for a plain
  ...
  unchanged. `Map<K, V>`/`Set<T>` are also supported: a fresh `Map`/`Set`
  is built and each entry's value/element is itself recursively cloned
  through this same dispatch (so a `Map<string, number[]>`'s array values
  are real, independent copies too), snapshotting the source via the same
  conversion `[...map]`/`Array.from(map)` already use; `K`/`T` must be
  `number` or `string` (an object/array/etc. key is a compile-time error --
  cloning a `Map` keyed by a fresh, differently-identitied object copy
  raises questions this compiler's `Map`/`Set` model has no answer for).
  Unlike QuickJS's version, `Function`, `Promise`, and a top-level
  `Json`/`Dictionary`/optional value are not supported here and fail to
  compile -- there is no cycle detection either, but a statically-typed
  value can't describe a cycle to begin with
...
The Brotli, TLS, and WebAssembly/WASI hosts are separate Cargo features and are
omitted from QuickJS fallback artifacts whose bundles do not reference them;
gzip and deflate remain available in the minimal host.
...
`Math.abs/min/max/floor/ceil/trunc/round/sqrt`, `cbrt/sign/hypot`,
`clz32/fround/imul`, logarithmic
and exponential functions, and trigonometric/hyperbolic functions (including
`atan2`), plus nested
...
Numeric compound assignments, numeric prefix/postfix updates, and string `+=`
reuse that write path while JIT duplication instructions evaluate the receiver
and index once.
...
Optional primitive, primitive-array, fixed-object, and tuple receivers can use
supported property reads and methods through `?.`. Fixed aggregates expand one
...
`number[]` `reduce`/`reduceRight` calls with a direct arithmetic callback scan
native storage in the corresponding direction in the JIT. `+`, `-`, `*`, `/`,
...
A fixed primitive tuple has its own native pointer tag, retaining typed index
...
runtime value. The same narrowing now feeds `.at()`, `.includes()`,
...
Object and tuple literal destructuring materializes every leaf once in source
order, including unused side-effecting leaves, while requested primitive-array
...
`while`, `do...while`, and classic `for (; test; update)` bodies may also return
a fixed aggregate early. A structured JIT result region carries every leaf out
of the loop and joins it with the post-loop fallback return without exiting the
...
Logical `&&` and `||` use explicit short-circuit regions in typed JIT IR. The
...
- The typed AOT class subset supports constructors, parameter properties,
  ...
  to identifiers use the same path, including across exports. A distinct class-expression
  self-name is retained inside constructors, methods and type annotations without leaking as a
  top-level binding. Computed fields, methods, accessors and
  ...
  method result types through direct receivers and instance `this`, including inherited members.
...
`some`, `every`, and generic-rest `sortBy` calls (including native array results
from array and homogeneous object collections) are covered. Typed non-callable package
...
completes the Engine.IO open and Socket.IO namespace handshake, then verifies
a compiled connection listener by echoing an event through the standalone
server (`registry_add_runs_a_real_socket_io_echo_when_enabled`).
